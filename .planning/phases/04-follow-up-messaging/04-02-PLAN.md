...
must_haves:
  truths:
    ...
    - "Cancelled appointments have their reminders removed from queue"
  artifacts:
  ...
  key_links:
    ...
    - from: "backend/src/services/divine/reminder-scheduler.service.ts"
      to: "backend/src/db/schema/messageQueue.ts"
      via: "cancelAppointmentReminders query"
      pattern: "db\\.update.*messageQueue"
---

<objective>
Implement scheduled appointment reminders (24h and 1h before) with booking and portal links.

Purpose: Per CONTEXT.md, customers should receive reminder messages 24 hours and 1 hour before their appointment, including rescheduling options. This improves show rates and customer experience. Also supports cancellation when appointments are cancelled/rescheduled.

Output: Reminder scheduler service that creates future-dated queue entries for both SMS and email reminders, with full cancellation support.
</objective>
...
<task type="auto">
  <name>Task 2: Create Reminder Scheduler Service with Cancellation Support</name>
...
Key implementation details for cancelAppointmentReminders:
1. Query pending messages with messageType of 'reminder_24h' or 'reminder_1h'
2. Filter by appointmentId stored in metadata
3. Update status to 'cancelled' for all matching messages
4. Return count of cancelled reminders
5. Log the operation for debugging
  </action>
  <verify>TypeScript compiles. Service exports reminderSchedulerService. cancelAppointmentReminders actually queries and updates the database.</verify>
  <done>reminder-scheduler.service.ts created with scheduleAppointmentReminders and fully implemented cancelAppointmentReminders methods</done>
</task>
...
4. Verify cancelAppointmentReminders has actual database query implementation (not TODO stub)
5. Verify exports from index.ts include reminderSchedulerService
</verification>
...
- reminderSchedulerService.cancelAppointmentReminders queries database and cancels pending reminders by appointmentId
