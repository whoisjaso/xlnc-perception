...
must_haves:
  ...
  artifacts:
    ...
    - path: "backend/src/services/divine/queue-processor.service.ts"
      provides: "Fixed retry intervals"
      contains: "60 * 1000"
  key_links:
...
Purpose: The existing message queue schema lacks fields for message type classification, cost tracking, and dead letter handling. This plan adds these fields and creates the business hours utility needed for marketing message timing.

Output: Enhanced schema with migration, business hours utility, and fixed retry intervals.
</objective>
...
DO NOT modify the existing fields - only add new ones.
  ...
  <done>Schema file includes messageType, costCents, deadLetterAt, deadLetterReason fields with proper types</done>
</task>
...
<task type="auto">
  <name>Task 3: Fix Retry Intervals to 1-Minute Fixed</name>
  <files>backend/src/services/divine/queue-processor.service.ts</files>
  <action>
Per CONTEXT.md decision: "Retry attempts: 3 attempts with fixed 1-minute intervals"
...
Also update the handleFailure method:
1. When attempts >= maxAttempts, set status to 'dead_letter' instead of 'failed'
2. Set deadLetterAt timestamp
3. Set deadLetterReason to the error message

Update the getStats method to count 'dead_letter' status separately from 'failed'.

Update QueueStats interface to include `deadLetter: number`.
  </action>
...
- deadLetterAt and deadLetterReason populated</verify>
  <done>Queue processor uses 1-minute fixed retry intervals and properly handles dead letter status</done>
</task>
...
4. Review queue-processor.service.ts retry logic
</verification>

<success_criteria>
- Message queue schema has messageType, costCents, deadLetterAt, deadLetterReason fields
