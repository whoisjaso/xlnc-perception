...
export function ReviewStep({ draft, next, back, progress, vehicle, settings }: StepProps & { vehicle: Vehicle; settings: CompanySettings }) {
  ...
  const rows: [string, string, StepId][] = [
    ...
    ['Renter', fullName(draft), 'name'],
    ['Contact', `${draft.phone}, ${draft.email}`, 'contact'],
    ['License', `${draft.license.state} ${draft.license.number}, expires ${draft.license.expires}`, 'license'],
    ['Address', formatAddress(draft.address), 'address'],
    ['Insurance', draft.insurance.mode === 'company-cdw' ? 'Our damage waiver' : `${draft.insurance.carrier} ${draft.insurance.policyNumber}${draft.insurance.mode === 'both' ? ' plus waiver' : ''}`, 'insurance'],
