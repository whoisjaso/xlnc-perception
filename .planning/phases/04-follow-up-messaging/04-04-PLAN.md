...
<task type="auto">
  <name>Task 1: Add Scheduled Messages and Dead Letter API Endpoints</name>
  <files>backend/src/routes/divine.ts, backend/src/services/divine/queue-processor.service.ts</files>
  <action>
Add new endpoints to retrieve scheduled messages and dead letter messages.

In queue-processor.service.ts, add getDeadLetterMessages method:
...
<task type="auto">
  <name>Task 3: Add WebSocket Integration and Connection Status to MessageQueueViewer</name>
  <files>components/divine/MessageQueueViewer.tsx</files>
  <action>
Add WebSocket integration to the existing MessageQueueViewer component.
...
3. Add WebSocket connection status indicator near the header:
```typescript
...
4. Add real-time event feed section (collapsible, at bottom or sidebar):
```typescript
...
Focus this task on WebSocket integration only. Keep existing tab/view structure.
  </action>
  <verify>Component renders without errors. WebSocket shows connected status. Real-time events appear in feed.</verify>
  <done>MessageQueueViewer has WebSocket integration with connection status and live event feed</done>
</task>

<task type="auto">
  <name>Task 4: Add Status Tabs, Alerts, and Scheduled View to MessageQueueViewer</name>
  <files>components/divine/MessageQueueViewer.tsx, src/services/divine.ts</files>
  <action>
Enhance the MessageQueueViewer with status tabs, alerts, and scheduled view.

1. Add new state for tabs and additional data:
```typescript
...
2. Add API methods to src/services/divine.ts:
```typescript
getDeadLetterMessages: async (clientId?: string) => {
  const response = await fetch(`${API_BASE}/divine/queue/dead-letter${clientId ? `?clientId=${clientId}` : ''}`, {
    headers: getAuthHeaders(),
  });
  return response.json();
},

getScheduledMessages: async (options?: { hours?: number; clientId?: string }) => {
  const params = new URLSearchParams();
  if (options?.hours) params.set('hours', options.hours.toString());
  if (options?.clientId) params.set('clientId', options.clientId);
  const response = await fetch(`${API_BASE}/divine/queue/scheduled?${params}`, {
    headers: getAuthHeaders(),
  });
  return response.json();
},
...
3. Update data loading to fetch dead letter and scheduled messages:
```typescript
const loadData = async () => {
  // ... existing fetches ...
  const [deadLetterData, scheduledData] = await Promise.all([
...
4. Add prominent failed/dead letter alert banner at top:
```typescript
...
// Render tabs with counts and active state
```

6. Add scheduled messages view when scheduled tab active:
```typescript
{activeTab === 'scheduled' && (
  <div className="space-y-2">
    ...
    {scheduledMessages.length === 0 ? (
      <div className="text-gray-500 text-sm">No scheduled messages</div>
    ) : (
        ...
        <div key={message.id} className="bg-black/20 p-3 border border-white/5">
          <div className="flex justify-between items-start">
            <div>
              ...
              <span className="text-[10px] text-gray-400">{message.recipient}</span>
            </div>
...
Import AlertCircle and Calendar from lucide-react.
  </action>
  <verify>Tabs switch correctly. Failed alert banner appears when there are failures. Scheduled view shows upcoming messages.</verify>
  <done>MessageQueueViewer has status tabs (All/Pending/Sent/Failed/Dead Letter/Scheduled), prominent failed alert banner, and scheduled messages view</done>
</task>
...
6. Verify scheduled tab shows upcoming messages
</verification>
...
- Scheduled messages view shows next 48 hours
